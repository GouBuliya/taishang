...
def get_orders(instrument_id:str=None):
    ...
    ans_algo={}
    res=tradeAPI.order_algos_list(instId=instrument_id) 
    if res and res.get('code') == '0' and res.get('data') and len(res['data']) > 0:
        algo_data=res['data'][0]
        ans_algo['instId']=algo_data.get('instId', "N/A")#策略id
        ans_algo['algo_state']=algo_data.get('state', "N/A")#策略状态
        ans_algo['algo_type']=algo_data.get('type', "N/A")#策略类型
        ans_algo['algo_trigger_price']=algo_data.get('triggerPx', "N/A")#触发价格
        ans_algo['algo_trigger_price']=algo_data.get('triggerPx', "N/A")#触发价格
