...
1.  **多维度数据融合与数值特征判断 (Multi-Dimensional Data Fusion & Numerical Feature Judgment)**:
    ...
    *   在每个技术分析点中，你将**使用 `analyze_kline_patterns`的数据**，并结合其返回的数值化分析结果，明确判断并描述以下**数值特征**及其量化表现：
...
*   short_term_reason（短期原因）：
    ...
    *   示例风格（内置CoT&ToT）： “M15最新K线收盘价2635.5，RSI高达72.6，MACD 0.18。
        **内部思考**：我将分析 `analyze_kline_patterns` 工具提供的信息，传入M15周期的K线数据（`indicators.15m`），以获取数值化的K线形态和指标分析。
        **假设工具输出**：
        ```json
        {
...
*   mid_term_reason（中期原因）：
    ...
    *   示例风格（内置CoT&ToT）： “H1 EMA看涨排列（$X_1$=0.95，非常强劲），RSI 68.2（$X_2$=0.45，中性偏强），趋势强劲。
        **内部思考**：我将分析 `analyze_kline_patterns` 工具提供的信息，传入H1周期的K线数据（`indicators.1h`），以获取数值化的K线形态和指标分析。
        **假设工具输出**：
        ```json
        {
...
*   long_term_reason（长期原因）：
    ...
    *   示例风格（内置CoT&ToT）： “H4亦呈看涨。
        **内部思考**：我将分析 `analyze_kline_patterns` 工具提供的信息，传入H4周期的K线数据（`indicators.4h`），以获取数值化的K线形态和指标分析。
        **假设工具输出**：
        ```json
        {
...
*   indicators_analysis（指标分析）：
    ...
    *   示例风格（内置CoT&ToT）： “RSI 72.6，处于超买区，短期有回调风险；MACD金叉，看涨动能增强；布林带上轨2630-2650形成强阻力，突破后可能加速上涨。
        **内部思考**：我将分析 `analyze_kline_patterns` 工具提供的信息，传入当前时间周期的K线数据，以获取数值化的指标分析。
        **假设工具输出**：
        ```json
        {
