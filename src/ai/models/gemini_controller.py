...
from src.data.analysis.kline_pattern_analyzer import analyze_kline_patterns # 确保这里导入了函数
...
MODULE_TAG = "[Controller_api_caller] "
...
SYSTEM_PROMPT_PATH = config["MODEL_CONFIG"]["Controller_SYSTEM_PROMPT_PATH"]
...
# ===================== 全局状态封装 =====================
# TODO: 将所有全局状态（计数器、计时器、配置等）封装到一个或多个配置/状态类中。
# 这将极大地提高代码的可测试性和可维护性，消除对全局变量的依赖。
...
def call_gemini_api_stream(
    ...
    Yields:
        Generator[Dict[str, Any], None, None]: 一个生成器，逐块返回API的响应。
                                                每个块是一个字典，可能包含 'text', 'thought', 'executable_code' 等键。
    """
    global cnt
    ...
    try:
        # Gemini API 配置
        # TODO: 将这些默认配置移到 `config.json` 中，使它们更易于调整。
...
def run_main_flow():
    ...
    # 1. 加载配置和数据
    # TODO: 这些路径应该通过PathManager获取，而不是在这里重新计算。
    system_json_path = config["data_path"]
    system_prompt_path = config["MODEL_CONFIG"]["Controller_SYSTEM_PROMPT_PATH"]
    
    logger.info(f"API Key in use: ...{_api_key[-4:]}")
    ...
    try:
        ...
        logger.error(f"加载或解析数据文件失败: {system_json_path}, 错误: {e}")
        return
    ...
    if args.append_prompt:
        logger.info(f"附加提示词: {args.append_prompt}")
        current_prompt_text += f"\n\n{args.append_prompt}"

    # 2. 准备截图文件
    files_path = []
    screenshots_config = data_json.get("screenshots", {})
    if not screenshots_config:
        logger.warning(f"{MODULE_TAG}配置中未找到截图文件路径")
    else:
        for key in ["15", "60", "240"]:
            if key in screenshots_config:
                files_path.append(screenshots_config[key])
            else:
                logger.warning(f"{MODULE_TAG}截图文件路径配置中缺少key: {key}")
    ...
    try:
        ...
        logger.error(f"无法清空日志文件: {EFFECTIVE_LOG_FILE}, 错误: {e}")
        return
    ...
    # 5. 保存结果
    if result:
        logger.info(f"{MODULE_TAG}收到最终模型响应，准备保存...")
...
def call_and_process_api(prompt_text, system_prompt_path, files_path, think_mode: bool = False):
    ...
    response_parts = []
    try:
        api_stream = call_gemini_api_stream(
        ...
        if not api_stream:
            logger.error(f"{MODULE_TAG}未获得API响应流")
            return ""
        ...
        for chunk in api_stream:
            if "text" in chunk:
                ...
                logger.error(f"{MODULE_TAG}API响应错误: {chunk['error']}")
...
def save_api_result(result: str):
    """解析并保存API的最终结果"""
    try:
        # 尝试提取和解析 JSON
        # TODO: 这个JSON提取逻辑可以更健壮，以处理更多边缘情况。
        if "```json" in result:
        ...
        output_file_path = config["Controller_answer_path"]
        try:
            with open(output_file_path, "w", encoding="utf-8") as f:
                json.dump(output_data, f, indent=4, ensure_ascii=False)
            print(f"{MODULE_TAG}已将解析后的输出内容保存到 {output_file_path}")
        except IOError as e:
            logger.error(f"{MODULE_TAG}保存输出内容时发生错误: {str(e)}")
    except json.JSONDecodeError:
        logger.error(f"{MODULE_TAG}解析响应为JSON失败")
        print(f"{MODULE_TAG}错误：无法解析模型响应为JSON")
        print(f"原始响应内容：{result}")

def print_performance_stats(run_time: float):
    """打印性能统计数据"""
    print("\n--- 性能统计 ---")
