...
execution = gemini_answer["execution_details"]
...
def close_position(instId: str, tdMode: str,size: float):
    ...
    try:
            type = "long" if (float(position_info['data'][0]['pos']) > 0) else "short"
            if type == "long":
            # 平仓
                ...
                logger.info(f"平仓结果: {result_}")
            else:
                logger.info("当前多单持仓为空,查看挂单信息") 
    except Exception as e:
            logger.error(f"获取当前持仓失败: {e}")
            return {"status": "error", "message": "获取当前持仓失败"}
    try:
    ...
    try:
        #如果是全部平仓
        order_size = order_info['data'][0]['sz']

        if (float(order_size) == float(size)):
            tradeAPI.cancel_order(instId=instId, ordId = order_info['data'][0]['ordId'])

            logger.info("全部平仓")
        else:#修改订单

            logger.info(f"挂单数量: {order_size}")
            #转化为float
            order_size = float(order_size)
            #转化为int
            tradeAPI.amend_order(instId=instId, ordId = order_info['data'][0]['ordId'],  newSz=order_size-size)#新的数量等于原有数量减去size
            logger.info(f"修改挂单数量: {order_size-size}")
...
def trade_api(execution):
    ...
    if order_type == "buy" or order_type == "sell":
        ...
        logger.info("类型为 close，需要实现平仓逻辑。")
        try:
            close_position(
                instId="ETH-USDT-SWAP",
                tdMode="cross",
        ...
        return {"status": "close", "message": "Close logic not implemented"}
...
if __name__ == "__main__":
    ...
    trade_result = trade_api(execution)
    logger.info(f"trade_result: {trade_result}")
    execution_json = json.dumps(execution, indent=4, ensure_ascii=False)
